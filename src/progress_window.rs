...
impl ProgressWindow {
    pub fn new(title: &str, total_files: usize) -> Result<Self> {
        unsafe {
            ...
            // Create window
            let window_class = Class::get("NSPanel").ok_or_else(|| eyre::eyre!("NSPanel class not found"))?;
            let window: id = msg_send![window_class, alloc];
            
            let frame = NSRect::new(
                NSPoint::new(0.0, 0.0),
                NSSize::new(500.0, 120.0),
            );
            ...
            // Create progress bar
            let progress_class = Class::get("NSProgressIndicator").ok_or_else(|| eyre::eyre!("NSProgressIndicator not found"))?;
            ...
            // Create text field
            let textfield_class = Class::get("NSTextField").ok_or_else(|| eyre::eyre!("NSTextField not found"))?;
    ...
    pub fn update(&self, file_index: usize, total_files: usize, file_name: &str, progress_text: String) {
        unsafe {
            let message = format!("File {}/{}:  {}\n{}", 
                file_index + 1, total_files, file_name, progress_text);
            
            if let (Ok(text_field), Ok(progress_bar)) = (self.text_field.lock(), self.progress_bar.lock()) {
...
impl ProgressWindow {
    ...
    pub fn update(&self, _file_index: usize, _total_files: usize, _file_name: &str, _progress_text: String) {
    }
