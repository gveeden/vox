...
fn check_and_offer_move_to_applications() -> Result<()> {
    ...
    let result = show_dialog(
        ...
        &message,
        &["Move to Applications", "Keep Here"],
    );
...
fn main() -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        unsafe {
            ...
            let red_color = NSColor::colorWithRed_green_blue_alpha_(nil, 1.0, 0.0, 0.0, 0.0);
    ...
    let status_item = MenuItem::new(
        format!("Hotkey: {}+{}", config.modifier_key, config.trigger_key),
        false,
        None,
    );
    let model_item = MenuItem::new(
        format!(
            "Model: {}",
            model_path_str.split('/').last().unwrap_or(&model_path_str)
        ),
        false,
        None,
    );
    ...
    let manager =
        GlobalHotKeyManager::new().wrap_err("Failed to initialize global hotkey manager")?;
    ...
    println!(
        "🎯 Target sample rate: {} Hz (will resample if needed)",
        target_sample_rate
    );
    ...
    println!(
        "🚀 Ready! Press {}+{} to start recording, press again to stop.",
        config.modifier_key, config.trigger_key
    );
        ...
        if let Ok(event) = hotkey_rx.try_recv() {
            if event.id == hotkey_id {
                ...
                if !is_currently_recording {
                    ...
                    if let Some(monitor) = recording_window_for_hotkey
                        .current_monitor()
                        .or_else(|| recording_window_for_hotkey.primary_monitor())
                    {
                        ...
                        let x = monitor_position.x + screen_size.width as i32
                            - window_width_physical
                            - 25;
...
fn process_recording_tdt(parakeet: &mut ParakeetTDT, job: TranscriptionJob) -> Result<()> {
    ...
    // Save transcript
    let transcript_path =
        PathBuf::from(&job.output_dir).join(format!("transcript_{}.txt", timestamp));
